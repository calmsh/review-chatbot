...
create policy "Messages are insertable by everyone" on messages
  for insert with check (true);
-- Add DELETE policies to allow public users to delete chats
create policy "Chats are deletable by everyone" on chats
  for delete using (true);

create policy "Messages are deletable by everyone" on messages
  for delete using (true);
